...
# 폴더 안의 .json 파일을 자동으로 전부 찾아서 업로드하는 방식
...
def find_raw_json_files() -> list[Path]:
    ...
    if not json_files:
        raise FileNotFoundError(f"업로드할 JSON 파일이 없습니다: {RAW_DATA_DIR}")
...
def upload_raw_files_to_s3() -> None:
    ...
    for raw_file in find_raw_json_files():
        raw_data = load_json(raw_file)

        if "owner" not in raw_data or "repo" not in raw_data:
            print(f"건너뜀: owner/repo 필드 없음 → {raw_file}")
            continue
